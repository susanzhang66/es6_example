...
// 父类的静态方法，也会被子类继承。继承在b._prototype_
//super这个关键字，既可以当作函数使用，也可以当作对象使用。
// super作为函数时，super()只能用在子类的构造函数之中，用在其他地方就会报错。
// super作为对象时，在普通方法中，指向父类的原型对象；在静态方法中，指向父类。
//     在子类普通方法中通过super调用父类的方法时，方法内部的this指向当前的子类实例。
//     总结：静态方法中用super对应的都是实体，不是实例。静态方法staticf中调用 super的父类方法时，指向是父类，不是父类原型。且运行环境是 子类，不是子类实例。
...
/**
 * @description: 子类的静态方法中通过super调用父类的方法时，方法内部的this指向当前的子类，而不是子类的实例。
 * @param {type} 
 * @return: 
 */  
