...
- 存值函数和取值函数是设置在属性的 **Descriptor 对象上的。** 有点迷茫 ----
- 类的属性名，可以采用表达式。
- 类也可以使用表达式的形式定义。
- 采用 Class 表达式，可以写出立即执行的 Class。

**静态方法**：
  - 一个方法前，加上static关键字，就表示该方法不会被实例继承。会被子类继承，或者可以被super调用。
  - 父类的静态方法（Static），可以被子类继承。
  - 静态方法也是可以从super对象上调用的。

**实例属性的新写法**: 写在class顶部。而且最外部是可以不用加this. (如：_counter = 1)
...
- （4）Generator 方法
- （5）this 的指向： 使用箭头函数。箭头函数内部的this总是指向**定义时所在的对象。**
...
class Point {
  ...
  class MyClass {
    ...
    get prop() {   //对prop属性 设置 取的 拦截函数
      return 'getter';
    }
    set prop(value) {   //对prop属性 设置 拦截函数
